...
DATA_SECTION
 ...
 END_CALCS
  ...
  matrix ldata(1,nl_rcrds+20000,1,5)
  vector ta_ubs(1,natows);
  vector a_ubs(1,na_rcrds+2000);
  vector l_ubs(1,nl_rcrds+20000);
  ...
  !! }
  !! cout<<"Nlfrq: "<<sum(nlfrq_sam_tow)<<endl;
  !! cout<<"Nfshl: "<<sum(nfshl_sam_tow)<<endl;
  // !! nsam_max=max(nlfrq_sam_tow);
  !! nsam_max=max(nfshl_sam_tow);
 ...
 LOCAL_CALCS
  l_recnum.initialize();
  cout << "N sample tows "<<nlfrq_sam_tow(l_tows(nl_rcrds)) <<" Nltows "<<nltows<<endl;
  int k=1;
  for (i=1;i<=nltows;i++) 
  {
  ...
  cout<< l_recnum(nltows,nlfrq_sam_tow(nltows))<<endl;
  cout<<"l_recnum final: "<< l_recnum(nltows,nlfrq_sam_tow(nltows)) <<endl;
 END_CALCS
 ...
 // Vess  Area  Depth Month Day yy  lat long  age sex spp wt  length  specno  samplesystem
    ...
    */
    ...
    for (i=1;i<=nltows*sam_level;i++)
    {
      ...
      for (int j=1;j<=(nlfrq_sam_tow(ii));j++) 
      {
        ...
        {
          ldata(ik)(1,4) = ldata_in(l_recnum(ii,j))(1,4);
          ldata(ik,5) = 1;
          ik++;
        }
        k++;
    ...
    cout <<"Bootstrap length records: "<< n_bsl_rcrds<<" "<<nl_rcrds<<endl;// exit(1);
    ...
    */
    // for (i=1;i<=nl_rcrds;i++) // Change to bootstrap N
       // dvector ttt(1,10);
       // dvector ppp(1,10); ppp(1)=.1; ppp(2)=.1; ppp(3)=.1; ppp(4)=.1; ppp(5)=.1; ppp(6)=.1; ppp(7)=.1; ppp(8)=.1; ppp(9)=.1; ppp(10)=.1;
       // ttt.fill_multinomial(rng, ppp); cout <<ttt <<endl; exit(1);
