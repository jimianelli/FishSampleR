...
cd        <- (read.csv(paste("imported/akfin_catch.csv",sep=""),as.is=T,header=T))
head(cd)
...
names(cd)
cdf <- cd %>% filter(Year==2017,FMP_Area=="BSAI",FMP_Subarea=="BS",Species_Group=="Pollock") %>% transmute(
...
wds <- wd[as.numeric(age)>2 & yr>2010,.(avg=mean(wt),cv=sd(wt)/mean(wt)),.(yr,age)] 
wds <- wd[as.numeric(age)>2 ,.(avg=mean(wt),cv=sd(wt)/mean(wt)),.(yr,age)] 
wds$Year <- as.factor(wds$yr)
ggplot(wds,aes(x=age,y=avg,shape=Year)) + mytheme + geom_point()
wds
...
{
  p <- dtmp[yr %between% c(i,i) &age>2&age<11] %>% ggplot(aes(x=as.factor(age),y=swt,fill=as.factor(yr-age))) 
...
i=2011
summary(dtmp)
for (i in c(1997,2002,2007,2012 ,2017))
{
